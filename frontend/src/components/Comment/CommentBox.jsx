...
const CommentBox = ({
  ...
  console.log('commet ', comment.items);
  ...
  return (
    <div>
      <div className={comment._id === 1 ? "inputContainer mb-10" : "commentContainer "}>
        {comment._id === 1 ? (
          <>
            <textarea
              rows={3}
                ...
                <div className="max-w-96 w-fit">
                  <div className="bg-gray-700 px-3 py-2 rounded-lg">
                    <div className="flex w-full items-center gap-2 mb-2">
                      <p className="text-start md:text-lg text-sm font-bold">{comment?.author?.name || 'hello world'}</p> {/*{comment.author.name}*/}
                      <p className="text-xs text-gray-500 text-start">1 days ago</p> {/*{formatDistanceToNow(new Date(comment.createdAt))}*/}
                    </div>
                    {/* comment box */}
                    <span
                      ...
                      <>
                        <div className="w-fit flex">
                          ...
                          <Action
                            ...
                            handleClick={() => {
                              if (inputRef.current)
                                inputRef.current.innerText = comment.body;
                              setEditMode(false);
                            }}
                          />
                        </div>
      ...
      <div style={{ display: expand ? "block" : "none", paddingLeft: 25 }}>
        ...
        { comment.items && comment?.items?.map((cmnt) => {
          // console.log('enter cmnt : ', cmnt._id);
          return (
            <CommentBox
              key={cmnt._id}
